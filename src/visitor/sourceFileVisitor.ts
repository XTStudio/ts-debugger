...
export class SourceFileVisitor implements BaseVisitor<ts.SourceFile> {

    test = (node: ts.Node) => ts.isSourceFile(node);

    visit(ctx: ts.TransformationContext, sourceFile: ts.SourceFile, node: ts.SourceFile): { node: ts.Node; forceReturn?: boolean | undefined; } {
